(ns onetrust.main
  "Kotodama WASM entrypoint for the Onetrust clean-room actor (L4) — Clojure port.
  ...
  facts (`onetrust.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "onetrust")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"onetrust.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "onetrust-compat" :tier tier :entities entities} 200])
...
(defn start! [] :onetrust-compat/ready)
